...
const { GastoServicio } = require('./models');
const { connectDB, onConnectionEvents, isConnected } = require('./mongo');

const queue = 'get.GastoServicio';
...
async function main() {
  ...
  console.log(`[💸] Servicio GastoServicio escuchando en ${queue}`);
  ...
  channel.consume(queue, async msg => {
    ...
    try {
      const docs = await GastoServicio.find({ casaID: { $in: ids } }).lean();
      channel.sendToQueue(
        msg.properties.replyTo,

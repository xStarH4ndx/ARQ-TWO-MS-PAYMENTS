...
async function main() {
  ...
  console.log(`[🏠] Servicio CasaPago escuchando en ${queue}`);
  ...
  channel.consume(queue, async msg => {
    ...
    try {
      const docs = await CasaPago.find({ casaID: { $in: ids } }).lean();
      channel.sendToQueue(
        msg.properties.replyTo,
